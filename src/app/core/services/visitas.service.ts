...
export class VisitasService {
  ...
  post(dto: Visita) {
    return this.http.post<Visita>(`${this.route}/crear`, dto).pipe(
      tap(() => {
  ...
  put(id: number, dto: Visita) {
    return this.http.put<Visita>(`${this.route}/actualizar/${id}`, dto).pipe(
      tap(() => {
        this._refreshListVisitas$.next(null);
      }),
      catchError(this.handleErrorService.handleError)
  ...
  validarVisitaPorSimpatizante(simpatizanteId: number) {
    return this.http.get<Visita>(
      `${this.route}/validar-visita-por-simpatizante/${simpatizanteId}`
    );
  }
}
